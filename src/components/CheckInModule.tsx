...
export const CheckInModule: React.FC = () => {
  ...
  // פונקציית עזר להמרת הערך מהמסד למלל מובן למאמן
  const getLimitationDisplay = (limitation: string) => {
    if (limitation === TrainingLimitation.PARTIAL) return 'בישיבה בלבד';
    if (limitation === TrainingLimitation.FULL) return 'בליווי פיזיותרפיסט';
    return 'מגבלה / לתשומת לב';
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 text-right" dir="rtl">
      ...
      <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
        ...
        {selectedResident && selectedResident.trainingLimitation !== TrainingLimitation.NONE && (
          <div className="bg-red-50 border-2 border-red-500 p-5 rounded-2xl mb-8 animate-pulse shadow-inner">
            <div className="flex items-center gap-3 text-red-700 mb-2">
              ...
              <div className="font-black text-xl italic">לתשומת לב המאמן: {getLimitationDisplay(selectedResident.trainingLimitation as string)}</div>
            </div>
            {selectedResident.medicalConditions && (
              <div className="mt-3 bg-white/80 p-3 rounded-xl border border-red-200 text-gray-800 font-bold">
                פירוט: {selectedResident.medicalConditions}
              </div>
            )}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-right">
          ...
          <div className="space-y-2 relative" ref={dropdownRef}>
            ...
            {isDropdownOpen && filteredResidents.length > 0 && (
              <div className="absolute z-50 w-full mt-2 bg-white border rounded-xl shadow-2xl max-h-72 overflow-auto">
                {filteredResidents.map(r => (
                  <div 
                    key={r.id} 
                    className="px-5 py-3 hover:bg-blue-50 cursor-pointer border-b"
                    onClick={() => {
        ...
        <Button 
          className="w-full mt-10 py-5 rounded-xl shadow-lg text-lg font-black" 
